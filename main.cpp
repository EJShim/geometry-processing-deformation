...
int main(int argc, char *argv[])
{
	...
	// Load input meshes
	igl::read_triangle_mesh((argc>1?argv[1]:"../data/decimated-knight.off"),V,F);
	U = V;
	...
	viewer.callback_mouse_down = [&](igl::opengl::glfw::Viewer&, int, int)->bool
	{
		...
		if(s.placing_handles)
		{
			...
			int fid;
			Eigen::Vector3f bary;
			if(igl::unproject_onto_mesh(last_mouse.head(2),viewer.core().view,viewer.core().proj, viewer.core().viewport, V, F, fid, bary))
			{
				long c;
				bary.maxCoeff(&c);
		...
		Eigen::MatrixXf CP;
		igl::project(Eigen::MatrixXf(s.CU.cast<float>()),viewer.core().view,viewer.core().proj, viewer.core().viewport, CP);
